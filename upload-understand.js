  ...
  btnConsolidate.addEventListener('click', async () => {
    ...
    try {
      ...
      const consolidatedProfile = {
        ...
        income: allIncome.map(x => ({ label: x.label || x.description || 'Income', amount: 'Rs ' + (Number(x.amount) || 0).toLocaleString('en-IN') })),
        incomeTotal: 'Rs ' + totalIncome.toLocaleString('en-IN'),
        expenses: allExpenses.map(x => ({ label: x.label || x.description || 'Expense', amount: 'Rs ' + (Number(x.amount) || 0).toLocaleString('en-IN') })),
